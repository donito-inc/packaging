...
The RPM spec file provided here is for Donu-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://donu.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Donu git repository](https://github.com/donu/donu) but are not part of
...
Currently only `Source10` is used, for the example `donu.conf` file.
...
in the donu git repository. At present only an SVG version of the Donu
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the donu
...
needed when 0.12.1 is released, a proper fix is already in the Donu git
master branch.
...
idea to build Donu Core against a static version of that library compiled
...
The `donu.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `donu-server` package. At this point, most RPM based
...
The `donu-server` package should have SELinux support. How to properly do
...
Donu-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/donu/). He has been
packaging Donu for Fedora at least since 2012.
...
4. I renamed the `donu` package that contains the Qt GUI to `donu-core` as
that appears to be how the general population refers to it, in contrast to
`donu-xt` or `donu-classic`. I wanted to make sure the general population
